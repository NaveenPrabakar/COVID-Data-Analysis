...
```{r}
missing_heatmap <- hospital %>%
  mutate(
    pct_missing = 100 - (Percentage.of.Required.Fields.Reported * 100)
  ) %>%
  group_by(State) %>%
  summarise(
    avg_missing = mean(pct_missing, na.rm = TRUE)
  )

ggplot(missing_heatmap, aes(x = State, y = 1, fill = avg_missing)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "white", high = "red",
                      name = "% Missing Data") +
  labs(
    title = "Average % Missing Data by State",
    x = "State",
    y = NULL
  ) +
  theme_minimal() +
...
>>>>>>> 058f93fb09dd7f22a98c95d4f09ce26d69b6d4dd
